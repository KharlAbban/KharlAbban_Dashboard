...
function setJumbotronStuff(locWeatherQuoteJson) {
	...
	weatherCont.querySelector(
		"[data-jumbotron-location]"
	).textContent = `${locWeatherQuoteJson.city}, ${locWeatherQuoteJson.country_code}`;
	weatherCont.querySelector(
		"[data-jumbotron-weather]"
	).textContent = `${locWeatherQuoteJson.mainWeatherInfo.temp}`;
	weatherCont.querySelector(
		"[data-jumbotron-feels]"
	).textContent = `${locWeatherQuoteJson.mainWeatherInfo.feels_like}`;
	document.querySelector(
		"[data-jumbotron-quote]"
	).textContent = `${locWeatherQuoteJson.quoteData[0].q}`;
	document.querySelector(
		"[data-jumbotron-author]"
	).textContent = `- ${locWeatherQuoteJson.quoteData[0].a}`;
}
...
async function fetchStuff() {
	...
	await fetch("/home", {
		...
		body: JSON.stringify(ipResponse),
	})
		.then((res) => res.json())
		.then((data) => {
...
dateTimeCont.querySelector("[data-jumbotron-date]").textContent =
	new Date().toDateString();
function setTime() {
	dateTimeCont.querySelector("[data-jumbotron-time]").textContent =
